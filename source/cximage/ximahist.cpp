...
bool CxImage::HistogramStretch(int32_t method, double threshold)
{
  ...
  if ((head.biBitCount==8) && IsGrayScale()){
	...
	for (yValue=0; yValue<head.biHeight; yValue++)
	{
		info.nProgress = (int32_t)(yValue*dbScaler);
		if (info.nEscape) break;
		for (x=0; x<head.biWidth; x++)	{
			p[BlindGetPixelIndex(x, yValue)]++;
		}
	}

	double maxh = 0;
	for (yValue=0; yValue<255; yValue++) if (maxh < p[yValue]) maxh = p[yValue];
	threshold *= maxh;
	int32_t minc = 0;
	...
	for (yValue=0; yValue<head.biHeight; yValue++)	{
		if (info.nEscape) break;
		info.nProgress = (int32_t)(50.0+yValue*dbScaler);
		for (x=0; x<head.biWidth; x++)
		{
			BlindSetPixelIndex(x, yValue, lut[BlindGetPixelIndex(x, yValue)]);
		}
	}
  } else {
	switch(method){
	case 1:
	  { // <nipper>
		...
		for (yValue=0; yValue<head.biHeight; yValue++)
		{
			...
			if (info.nEscape) break;
			for (x=0; x<head.biWidth; x++)	{
				RGBQUAD color = BlindGetPixelColor(x, yValue);
				p[color.rgbRed]++;
				p[color.rgbBlue]++;
				p[color.rgbGreen]++;
			}
		...
		for (yValue=0; yValue<head.biHeight; yValue++)	{
			...
			for (x=0; x<head.biWidth; x++)
			{
				...
				BlindSetPixelColor(x, yValue, color);
			}
		}
	  }
		break;
	case 2:
	  { // <nipper>
		...
		for (yValue=0; yValue<head.biHeight; yValue++)
		{
			info.nProgress = (int32_t)(yValue*dbScaler);
			if (info.nEscape) break;
			for (int32_t xValue=0; xValue<head.biWidth; xValue++)	{
				RGBQUAD color = BlindGetPixelColor(xValue, yValue);
				pR[color.rgbRed]++;
				pB[color.rgbBlue]++;
				pG[color.rgbGreen]++;
			}
		...
		double maxh = 0;
		for (yValue=0; yValue<255; yValue++) if (maxh < pR[yValue]) maxh = pR[yValue];
		...
		maxh = 0;
		for (yValue=0; yValue<255; yValue++) if (maxh < pG[yValue]) maxh = pG[yValue];
		...
		maxh = 0;
		for (yValue=0; yValue<255; yValue++) if (maxh < pB[yValue]) maxh = pB[yValue];
		threshold2 = threshold*maxh;
		int32_t minB = 0;
