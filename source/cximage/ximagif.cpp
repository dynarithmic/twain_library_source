...
int16_t CxImageGIF::decoder(CxFile* file, CImageIterator* iter, int16_t linewidth, int32_t &bad_code_count)
{
   ...
   /* Allocate space for the decode buffer */
	if ((localbuf = new uint8_t[linewidth + 1]) == NULL) return(OUT_OF_MEMORY);
    ...
    */
	while ((c = get_next_code(file)) != ending) {
		...
		if (c == clear){
			...
			*bufptr++ = (uint8_t)c;
			if (--bufcnt == 0) {
				if (iter) {
					if ((ret = (int16_t)out_line(iter, localbuf, linewidth)) < 0) {
						delete [] localbuf;
						return(ret);
					}
				}
	...
	if (bufcnt != linewidth && iter)
		ret = (int16_t)out_line(iter, localbuf, (linewidth - bufcnt));
	delete [] localbuf;
	return(ret);
}
////////////////////////////////////////////////////////////////////////////////
