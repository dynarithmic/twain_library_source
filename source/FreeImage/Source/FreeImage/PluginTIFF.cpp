...
static BOOL tiff_write_comment(TIFF *tiff, FIBITMAP* dib);
	...
	if(FreeImage_GetMetadataCount(FIMD_IPTC, dib)) 
	{
		...
		if(write_iptc_profile(dib, &profile, &profile_size)) 
		{
			...
			if (TIFFIsByteSwapped(tiff)) {
				TIFFSwabArrayOfLong((uint32_t *) iptc_profile.data(), (unsigned long)iptc_size/4);
			}
...
static BOOL
tiff_write_comment(TIFF *tiff, FIBITMAP* dib)
{
    FITAG *tag = NULL;
    ...
    if (tag) 
    {
        ...
        if (NULL != tag_value) 
        {
            TIFFSetField(tiff, TIFFTAG_SOFTWARE, tag_value);
            return TRUE;
        }
    }
    return FALSE;
