...
public:
  ...
  PageBlock(BlockType type = BLOCK_CONTINUEUS, int val1 = -1, int val2 = -1) : m_type(type), flags(0)
  {
  ...
  PageBlock& setFlags(int f) { flags = f; return *this; }
...
FreeImage_InternalGetPageCount(FIMULTIBITMAP *bitmap) {	
	if (bitmap) {
		if (((MULTIBITMAPHEADER *)bitmap->data)->handle) {
			...
			void *data = FreeImage_Open(header->node, &header->io, header->handle, TRUE, NULL, 0);
...
FreeImage_SaveMultiBitmapToHandle(FREE_IMAGE_FORMAT fif, FIMULTIBITMAP *bitmap, FreeImageIO *io, fi_handle handle, int flags) {
	...
	if (list) {
		...
		if(node) {
			...
			// dst data
			void *data = FreeImage_Open(node, io, handle, FALSE, NULL, flags);
			// src data
			void *data_read = NULL;
			...
			if(header->handle) {
				// open src
				header->io.seek_proc(header->handle, 0, SEEK_SET);
				data_read = FreeImage_Open(header->node, &header->io, header->handle, TRUE, NULL, flags);
			}
					...
					switch(i->m_type) {
						case BLOCK_CONTINUEUS:
						{
							for (int j = i->getStart(); j <= i->getEnd(); j++) {
								...
								// save the data
								success = node->m_plugin->save_proc(io, dib, handle, count, i->getFlags(), data);
								count++;
						...
						case BLOCK_REFERENCE:
						{
							...
							FIBITMAP *dib = FreeImage_LoadFromMemory(header->cache_fif, hmem, flags);
...
FreeImage_AppendPage(FIMULTIBITMAP *bitmap, FIBITMAP *data) {
	...
	if(const PageBlock block = FreeImage_SavePageToBlock(header, data, 0)) {
		// add the block
...
FreeImage_AppendPageEx(FIMULTIBITMAP *bitmap, FIBITMAP *data, int flags) {
    ...
    if (const PageBlock block = FreeImage_SavePageToBlock(header, data, flags)) {
        // add the block
        header->m_blocks.push_back(block);
        header->changed = TRUE;
        header->page_count = -1;
...
void DLL_CALLCONV
FreeImage_InsertPageEx(FIMULTIBITMAP *bitmap, int page, FIBITMAP *data, int flags) {
    if (!bitmap || !data) {
        return;
    }
    if (page >= FreeImage_GetPageCount(bitmap)) {
