...
fipImage::fipImage(const fipImage& Image) : _dib(nullptr), _fif(Image._fif), _bHasChanged(false)
{
...
BOOL fipImage::pasteSubImage(fipImage& src, int left, int top, int alpha)
{
    if(_dib)
    {
        const BOOL bResult = FreeImage_Paste(_dib.get(), src._dib.get(), left, top, alpha);
        _bHasChanged = TRUE;
        return bResult;
    }
    return FALSE;
...
void fipImage::setHorizontalResolution(double value) {
    FreeImage_SetDotsPerMeterX(_dib.get(), static_cast<unsigned>(value * 100 + 0.5));
}

void fipImage::setVerticalResolution(double value) {
    FreeImage_SetDotsPerMeterY(_dib.get(), static_cast<unsigned>(value * 100 + 0.5));
}
...
BOOL fipImage::clearThumbnail() {
    return FreeImage_SetThumbnail(_dib.get(), nullptr);
}
...
BOOL  fipImage::save(FREE_IMAGE_FORMAT fif, const char* lpszPathName, int flag) {
    const BOOL bSuccess = FreeImage_Save(fif, _dib.get(), lpszPathName, flag);
    _fif = fif;
    return bSuccess;
}
...
BOOL fipImage::save(const char* lpszPathName, int flag) {
    ...
    if(fif != FIF_UNKNOWN ) {
        ...
        if(image_type == FIT_BITMAP) {
            ...
            const WORD bpp = static_cast<WORD>(FreeImage_GetBPP(_dib.get()));
...
BOOL  fipImage::saveU(FREE_IMAGE_FORMAT fif, const wchar_t* lpszPathName, int flag) {
    const BOOL bSuccess = FreeImage_SaveU(fif, _dib.get(), lpszPathName, flag);
    _fif = fif;
    return bSuccess;
}
...
BOOL fipImage::saveU(const wchar_t* lpszPathName, int flag) {
    ...
    // Try to guess the file format from the file extension
    FREE_IMAGE_FORMAT fif = FreeImage_GetFIFFromFilenameU(lpszPathName);
    if(fif != FIF_UNKNOWN ) {
        // Check that the dib can be saved in this format
        BOOL bCanSave;
...
{
    const BOOL bSuccess = FreeImage_SaveEx(fif, _dib.get(), lpszPathName, page, flag);
    _fif = fif;
    return bSuccess;
}
...
BOOL fipImage::combineChannels(fipImage& red, fipImage& green, fipImage& blue) {
    if(!_dib) {
        const int width = red.getWidth();
        const int height = red.getHeight();
...
void fipImage::clearMetadata() {
    ...
    FreeImage_SetMetadata(FIMD_COMMENTS, _dib.get(), nullptr, nullptr);         // single comment or keywords
    FreeImage_SetMetadata(FIMD_EXIF_MAIN, _dib.get(), nullptr, nullptr);        // Exif-TIFF metadata
    FreeImage_SetMetadata(FIMD_EXIF_EXIF, _dib.get(), nullptr, nullptr);        // Exif-specific metadata
    FreeImage_SetMetadata(FIMD_EXIF_GPS, _dib.get(), nullptr, nullptr);         // Exif GPS metadata
    FreeImage_SetMetadata(FIMD_EXIF_MAKERNOTE, _dib.get(), nullptr, nullptr);   // Exif maker note metadata
    FreeImage_SetMetadata(FIMD_EXIF_INTEROP, _dib.get(), nullptr, nullptr);     // Exif interoperability metadata
    FreeImage_SetMetadata(FIMD_IPTC, _dib.get(), nullptr, nullptr);             // IPTC/NAA metadata
    FreeImage_SetMetadata(FIMD_XMP, _dib.get(), nullptr, nullptr);              // Abobe XMP metadata
    FreeImage_SetMetadata(FIMD_GEOTIFF, _dib.get(), nullptr, nullptr);          // GeoTIFF metadata
    FreeImage_SetMetadata(FIMD_ANIMATION, _dib.get(), nullptr, nullptr);        // Animation metadata
    FreeImage_SetMetadata(FIMD_CUSTOM, _dib.get(), nullptr, nullptr);           // Used to attach other metadata types to a dib
    FreeImage_SetMetadata(FIMD_EXIF_RAW, _dib.get(), nullptr, nullptr);         // Exif metadata as a raw buffer
}
