                ...
                bool expand_range(Container& ct) const
                {
                    if (m_isRange)
                    {
                        ...
                        if (API_INSTANCE DTWAIN_RangeExpand(m_theArray, &temp))
                        {
                            ...
                            auto pBuffer = temp_arr.get_buffer<dtwain_underlying_type<Container::value_type>::value_type>();
                            std::transform(pBuffer, pBuffer + sz, std::inserter(ct, std::end(ct)), []
                                (typename dtwain_underlying_type<Container::value_type>::value_type val)
