...
*/
#ifndef DTWAIN_DEVICEEVENTS_OPTIONS_HPP
#define DTWAIN_DEVICEEVENTS_OPTIONS_HPP
    ...
    namespace twain
    {
        class deviceevent_options
        {
            ...
            public:
                ...
                {
                    m_vDeviceEvents.clear();
                    std::copy(c.begin(), c.end(), std::back_inserter(m_vDeviceEvents));
                    return *this;
                }

                static const std::array<uint16_t, 1>& get_affected_caps()
                {
                    static std::array<uint16_t, 1> affected_caps = { CAP_DEVICEEVENT };
