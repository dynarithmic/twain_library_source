...
namespace TWAINDemo
{
    public partial class TestCapDlg : Form
    {
        ...
        private void SetTestSelection(string getType)
        {
            ...
            DTwainDemo.TwainAPI.DTWAIN_GetTwainNameFromConstant(TwainAPI.DTWAIN_CONSTANT_TWTY, bestDataType, szBestDataType, 100);
        ...
        private void SetTestSelection2(string setType)
        {
            ...
            if (DTwainDemo.TwainAPI.DTWAIN_GetCapOperations(m_Source, capToTestAsInt, ref capOpts) == 1)
            {
            ...
            int nID = DTwainDemo.TwainAPI.DTWAIN_GetConstantFromTwainName(setType);
        ...
        private void TestGetCap()
        {
            ...
            if (!bIsCapNameSupported && !bIsCustomCap)
            {
                ...
                int nValue = DTwainDemo.TwainAPI.DTWAIN_GetTwainNameFromConstant(TwainAPI.DTWAIN_CONSTANT_CAPCODE_NOMNEMONIC, 
                ...
                if ( bTranslate == 1 )
                {
                    ...
                    nTranslationID = -1;
                    bGotID = DTwainDemo.TwainAPI.DTWAIN_GetTwainNameFromConstant(
                    ...
                    switch (nArrayType)
                    {
                        ...
                        case TwainAPI.DTWAIN_ARRAYLONG:
                        {
                            ...
                            if ( capToTestAsInt == TwainAPI.DTWAIN_CV_CAPSUPPORTEDDATS )
                            {
                                ...
                                DTwainDemo.TwainAPI.DTWAIN_GetTwainNameFromConstant(TwainAPI.DTWAIN_CONSTANT_DG, hiWord, szTemp, 30);
                                DTwainDemo.TwainAPI.DTWAIN_GetTwainNameFromConstant(TwainAPI.DTWAIN_CONSTANT_DAT, loWord, szTemp2, 30);
                                szValues.AppendFormat("{0} / {1}", szTemp.ToString(), szTemp2.ToString());
                            }
                            else
                            if (bIsCapNameSupported)
                                DTwainDemo.TwainAPI.DTWAIN_GetNameFromCap(lVal, szValues, 256);
                            else
                            ...
                            // not the value
                            if (bGotID == 1)
                                DTwainDemo.TwainAPI.DTWAIN_GetTwainNameFromConstant(nTranslationID, lVal, szValues, 256);
                            else
                            {
                                string sPrefix = "";
        ...
        private void TestSetCap()
        {
            ...
            int nSetType = DTwainDemo.TwainAPI.DTWAIN_GetConstantFromTwainName(szSetType);
            // Get the container type 
            ...
            if (arrayType == TwainAPI.DTWAIN_ARRAYFRAME)
            {
                ...
                aValues = DTwainDemo.TwainAPI.DTWAIN_FrameCreate(frameValues[0], frameValues[1], frameValues[2], frameValues[3]);
            }
            else
            { 
                aValues = DTwainDemo.TwainAPI.DTWAIN_ArrayCreate(arrayType, 0);
                if ( arrayType == TwainAPI.DTWAIN_ARRAYANSISTRING )
                {
                    DTwainDemo.TwainAPI.DTWAIN_ArrayAddANSIString(aValues, szInput);
                }
                else
                {
                    switch(nDataType)
                    {
                        case TwainAPI.DTWAIN_TWTY_BOOL:
                        {
                            ...
                            DTwainDemo.TwainAPI.DTWAIN_ArrayAddLong(aValues, value);
                        }
                        break;
                        ...
                        case TwainAPI.DTWAIN_TWTY_UINT32:
                        {
                            ...
                            int value = DTwainDemo.TwainAPI.DTWAIN_GetConstantFromTwainName(items[0]);
                            if (value != int.MinValue)
                                DTwainDemo.TwainAPI.DTWAIN_ArrayAddLong(aValues, value);
                            else
                            {
                                if (string.IsNullOrWhiteSpace(items[0]))
                                    DTwainDemo.TwainAPI.DTWAIN_ArrayAddLong(aValues, 0);
                                else
                                    DTwainDemo.TwainAPI.DTWAIN_ArrayAddLong(aValues, SafeToInt32(items[0]));
                            }
                        }
                        break;
                        case TwainAPI.DTWAIN_TWTY_FIX32:
                            DTwainDemo.TwainAPI.DTWAIN_ArrayAddFloat(aValues, SafeToDouble(items[0]));
                        break;
                    }
                }
            }
            // Call the capability function 
            int ret = DTwainDemo.TwainAPI.DTWAIN_SetCapValuesEx2(m_Source, capToTestAsInt, nSetType, nContainerType, 
                                                      nDataType, aValues);
            DTwainDemo.TwainAPI.DTWAIN_ArrayDestroy(aValues);
            if (ret == 1)
